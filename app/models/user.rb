...
class User < ApplicationRecord
  ...
  scope :not_admin, -> { joins(:roles).where.not('roles.name = ?', 'admin') }

  scope :users_for_TL, -> { joins(:roles).where('roles.name = ? OR roles.name = ?', 'devloper', 'trainee') }

  scope :trainee, -> { joins(:roles).where('roles.name = ?', 'trainee') }
